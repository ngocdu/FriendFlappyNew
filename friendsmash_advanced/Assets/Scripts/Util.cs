...
public class Util : ScriptableObject
{
	...
	public static void FriendEnemyPictureCallback(FBResult result)
	{
		if (result.Error != null)
		{
			Debug.LogError(result.Error);
...
//		int i = GameStateManager.FriendTextureEnemys.Length;
		GameStateManager.FriendTextureEnemys[GameStateManager.indexFriendEnemy] = result.Texture;
	}

    public static Dictionary<string, string> RandomFriend(List<object> friends)
    {
		int r = Random.Range(0, friends.Count - 1);
		GameStateManager.Instance.numberFriendID = r;
        var fd = ((Dictionary<string, object>)(friends[r]));
	...
	//du them vao
	public static Dictionary<string, string> RandomFriendEnemy(List<object> friends)
	{
//		friends.Remove(friends[GameStateManager.Instance.numberFriendID]);
//		Debug.Log("number frienddi: " + GameStateManager.Instance.numberFriendID);
		int r = Random.Range(0, friends.Count - 1);
		while( r == GameStateManager.Instance.numberFriendID)
		{
			r = Random.Range(0, friends.Count - 1);
		}
		var fd = ((Dictionary<string, object>)(friends[r]));
		var friend = new Dictionary<string, string>();
		friend["id"] = (string)fd["id"];
